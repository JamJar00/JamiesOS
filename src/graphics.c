...
void clear_screen() {
    rect_t r = {0, display.width-1, 0, display.height-1};
    draw_fill_rectangle(r, display.background);
}
...
void draw_char(char c, uint16_t x, uint16_t y) {
    ...
    PGM_P fdata; 
    uint8_t bits, mask;
    uint16_t sc=x, ec=x + 4, sp=y, ep=y + 7;
    ...
    for(col=sc; col<=ec; col++) {
        ...
        for(row=sp, mask=0x01; row<=ep; row++, mask<<=1)
